...
func buildMetrics(out Summary, incidents []Incident, runDuration time.Duration, now time.Time) string {
	...
	for _, proj := range out.Projects {
		for _, c := range proj.Checks {
			...
			p.sample("status_check_up", ls, b2f(c.Status == "up"))
	...
	// Сводка по обходу. downCount на странице виден как «operational /
	// degraded / down»; в числах это удобнее сравнивать порогом.
	var up, total int
	for _, proj := range out.Projects {
		up += proj.Up
		total += proj.Total
	}
	p.family("status_checks_total", "Всего проверок в конфиге", "gauge")
	p.sample("status_checks_total", nil, float64(total))
	p.family("status_checks_up", "Проверок в состоянии up", "gauge")
