...
func main() {
	...
	// Пределы применяем сразу после чтения, ко всей карте. recordRelease
	// пройдёт только по целям из конфига, а в файле остаются и ключи
	// исчезнувших целей — раз ключ уже никогда не обновится, его список
	// изменений иначе останется в файле навсегда.
...
func recordRelease(history []Release, b OutBuild, now time.Time) []Release {
	...
	if len(history) > 0 && history[0].Version == b.Version {
		// Версия та же, но список изменений мог появиться позже записи о ней:
		// при переходе на новый формат releases.json головная запись уже
		// лежит на диске без changelog, а сервис его отдаёт. Дописываем.
		//
		// Только в пустое. Затирать уже сохранённый список тем, что сервис
		// отдаёт сейчас, нельзя: version.json могли перезаписать без поля,
		// а история — это то, что было, а не то, что видно в эту минуту.
		if len(history[0].Changelog) == 0 && len(b.Changelog) > 0 {
			history[0].Changelog = changelogField(b.Changelog)
		}
		return trimReleaseChangelogs(history)
	}
	history = append([]Release{{
		...
		Changelog: changelogField(b.Changelog),
	}}, history...)
