...
flowchart LR
    subgraph host["один хост"]
        svc["сервисы<br/>Лаунчер · Змейки · Метро<br/>Визитка · Статус · Мониторинг"]
...
запуск каждые пять минут — мельче cron у GitHub не умеет, — но cron там
просьба, а не обязательство: под нагрузкой запуски пропускаются пачками, и
замеренные промежутки между настоящими запусками за сутки составили от часа до
трёх с половиной. Обещать по такому расписанию нечего, поэтому вместо частоты
считается возраст: сводка прогона и сообщение коммита с данными говорят,
сколько сторожа не было, а перерыв длиннее шести часов уезжает в Telegram
отдельным сообщением. Без него молчащий сторож неотличим от спокойного —
последняя сводка лежит в ветке одинаково и через пять минут, и через полдня.
Пять обходов с минутным шагом внутри одного запуска остались: смене состояния
сторож верит только после двух обходов подряд, а одной точкой моргание от
падения не отличить.
...
(`config/status.json`, 12 проверок на шесть проектов): маркер в теле,
...
158 Go-тестов: 69 у агента (вердикты, масштаб сбоя, подтверждение сбоя,
разбор systemd, окна аптайма, журнал выкаток, метрики) и 89 у бота
