// Пробер статус-страницы: обходит сервисы, копит историю, ищет инциденты.
//
// Запускается из GitHub Actions каждые 5 минут (минимальный интервал cron у
// GitHub) и коммитит результат в data/. Работает снаружи инфраструктуры
// samoy.love — если сервер лежит, проверка это увидит и зафиксирует.
//
...
const config = JSON.parse(await readFile(join(ROOT, 'config/services.json'), 'utf8'));
